    ...
    <main className={`game-shell ${g.won ? "victory" : ""}`}>
      ...
      <div className="workspace v2">
        ...
        <aside className="shop-panel">
          ...
          <ShopRow name="Iron Drill" detail="+0.8 iron ore/s" icon={ASSET.ironOre} count={g.drills} priceIcon="◆" price={costs.drill} canBuy={g.ore >= costs.drill} onBuy={() => spend("ore", costs.drill, (s) => ({ ...s, drills: s.drills + 1 }))}/>
          <ShopRow name="Copper Drill" detail="+0.65 copper ore/s" icon={ASSET.copperOre} count={g.copperDrills} priceIcon="⚙" price={costs.copperDrill} canBuy={g.gears >= costs.copperDrill} onBuy={() => spend("gears", costs.copperDrill, (s) => ({ ...s, copperDrills: s.copperDrills + 1 }))}/>
          <div className="shop-group">PROCESSING</div>
          <ShopRow name="Iron Furnace" detail="+0.6 iron plate/s" icon={ASSET.ironPlate} count={g.furnaces} priceIcon="◆" price={costs.furnace} canBuy={g.ore >= costs.furnace} onBuy={() => spend("ore", costs.furnace, (s) => ({ ...s, furnaces: s.furnaces + 1 }))}/>
          <ShopRow name="Copper Furnace" detail="+0.5 copper plate/s" icon={ASSET.copperPlate} count={g.copperFurnaces} priceIcon="▰" price={costs.copperFurnace} canBuy={g.plates >= costs.copperFurnace} onBuy={() => spend("plates", costs.copperFurnace, (s) => ({ ...s, copperFurnaces: s.copperFurnaces + 1 }))}/>
          <div className="shop-group">ASSEMBLY</div>
          <ShopRow name="Gear Press" detail="2 iron plate → gear" icon={ASSET.gear} count={g.assemblers} priceIcon="▰" price={costs.assembler} canBuy={g.plates >= costs.assembler} onBuy={() => spend("plates", costs.assembler, (s) => ({ ...s, assemblers: s.assemblers + 1 }))}/>
